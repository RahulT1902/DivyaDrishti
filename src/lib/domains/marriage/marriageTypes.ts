...
// MarriageAssessment is the structured output of the Marriage Domain Engine.
//
...
export interface MarriageAssessment {
  domain: "Marriage";

  // ── Top-level state ────────────────────────────────────────────────────────
  currentState: string;    // "Highly Favorable" | "Favorable" | "Moderate" | "Challenging" | "Highly Challenging"
  confidence:   number;    // 0–100 overall marriage confidence
  ...
  marriagePotential:    number;  // 0–100
  partnerCompatibility: number;  // 0–100
  romanticHarmony:      number;  // 0–100
  marriageStability:    number;  // 0–100
  relationshipTiming:   number;  // 0–100
  ...
  // Fact → Inference → Hypothesis → Decision chain.
